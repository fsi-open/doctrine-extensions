...
use FSi\DoctrineExtensions\Tests\Versionable\Fixture\Category;
use FSi\DoctrineExtensions\Tests\Versionable\Fixture\Article;
use FSi\DoctrineExtensions\Tests\Versionable\Fixture\ArticleVersion;
...
{
    const CATEGORY = "FSi\\DoctrineExtensions\\Tests\\Versionable\\Fixture\\Category";
    const ARTICLE = "FSi\\DoctrineExtensions\\Tests\\Versionable\\Fixture\\Article";
    const ARTICLE_VERSION = "FSi\\DoctrineExtensions\\Tests\\Versionable\\Fixture\\ArticleVersion";
    ...
    public function testQueryWalker()
    {
        ...
        $query->setHint(\Doctrine\ORM\Query::HINT_CUSTOM_TREE_WALKERS, array('FSi\DoctrineExtensions\Versionable\Query\VersionableTreeWalker'));
        ...
        $query = $this->_em->createQuery("SELECT a FROM ".self::ARTICLE." AS a ORDER BY a.id");
        $query->setHint(\Doctrine\ORM\Query::HINT_CUSTOM_TREE_WALKERS, array('FSi\DoctrineExtensions\Versionable\Query\VersionableTreeWalker'));
        $this->setExpectedException('FSi\DoctrineExtensions\Versionable\Exception\RuntimeException');
        $articles = $query->execute();
    }
